...
class BaseModel extends BaseObject
{
			...
			if ($vs_field_value_is_null && !in_array($vs_field_type, array(FT_MEDIA, FT_FILE))) {
				...
				switch ($vs_field_type) {
					...
					case (FT_HISTORIC_DATERANGE):
						...
						if (!isset($this->_FIELD_VALUES[$start_field_name])) {
							$this->_FIELD_VALUES[$start_field_name] = null;
						}
						if (!isset($this->_FIELD_VALUES[$end_field_name])) {
							$this->_FIELD_VALUES[$end_field_name] = null;
						}
				...
				foreach ($va_possible_conflicts as $va_conflict) {
					...
					if (isset($va_conflict['snapshot']) && is_array($va_conflict['snapshot'])) {
						foreach ($va_conflict['snapshot'] as $vs_field => $vs_value) {
							if ($va_conflict_fields[$vs_field] ?? null) {
								continue;
							}
	...
	public function getMediaTag($ps_field, $ps_version, $pa_options = null)
	{
		...
		if (!is_array($va_media_info[$ps_version] ?? null)) {
			return null;
		}
	...
	public function &getMediaInfo($ps_field, $ps_version = null, $ps_property = null)
	{
		...
		if ($ps_version) {
			if (!$ps_property) {
				...
				if ($vs_v = ($va_media_info[$ps_version][$ps_property] ?? null)) {
					return $vs_v;
				}
				if ($vs_v = ($va_media_info[$ps_version][strtoupper($ps_property)] ?? null)) {
					return $vs_v;
				}
				if ($vs_v = ($va_media_info[$ps_version][strtolower($ps_property)] ?? null)) {
					return $vs_v;
				}
			}
		} else {
	...
	{
		if (defined('__CA_DONT_LOG_CHANGES__') || !$this->logChanges()) {
			return null;
		}
		...
		if (!in_array($change_type, ['I', 'U', 'D'])) {
			throw new ApplicationException(_t('Invalid log change type: %1', $change_type));
		...
		if ($is_metadata) {
			...
			if (($change_type !== 'D') && !caGetOption('forceLogChange', $options, false) && !sizeof($this->getChangedFieldValuesArray())) {
				return null;
		...
		if (is_array($subject_config)) {
			if (is_array($subject_config['FOREIGN_KEYS']) && $subject_tablename) {
				foreach ($subject_config['FOREIGN_KEYS'] as $field) {
					...
					if ($relationships['one_table']) {
						...
						if (!isset($subjects[$table_num]) || !is_array($subjects[$table_num])) {
							$subjects[$table_num] = [];
						}

						if ($is_metadata) {
							if (!$t->load($this->get('row_id'))) {
								return false;
							}
							$id = $t->get($field);
						} elseif ($is_metadata_value) {
							$t_attr = new ca_attributes($this->get('attribute_id'));
							if (!($xrow_id = $t_attr->get('row_id'))) {
								return false;
							}
							if (!$t->load($xrow_id)) {
								return false;
			...
			if (is_array($subject_config['RELATED_TABLES'])) {
				...
				foreach ($subject_config['RELATED_TABLES'] as $dest_table => $path_to_dest) {
					...
					if (!($t_dest = Datamodel::getInstance($dest_table))) {
						continue;
					}
					...
					if ($qr_subjects = $o_db->query($sql, [$xrow_id])) {
						if (!isset($subjects[$dest_table_num]) || !is_array($subjects[$dest_table_num])) {
							$subjects[$dest_table_num] = [];
						}
		...
		if (!sizeof($subjects) && !$log_changes_to_self) {
			return true;
		}
	...
	{
		if (!is_array($pa_hierarchy_data[$pn_id] ?? null)) {
			return [];
		}
	...
	public function addRelationship($pm_rel_table_name_or_num, $pn_rel_id, $pm_type_id = null, $ps_effective_date = null, $ps_source_info = null, $ps_direction = null, $pn_rank = null, $pa_options = null)
	{
		...
		if ($ps_direction) {
			$ps_direction = strtolower($ps_direction);
		}
		if (!in_array($ps_direction, ['ltor', 'rtol'])) {
			$ps_direction = 'ltor';
		}
		...
		if (is_array($one_to_many_relations)) {
			...
			foreach ($one_to_many_relations as $many_table => $info) {
				foreach ($info as $relationship) {
					...
					if ($many_many_only && !Datamodel::isRelationship($many_table)) {
						continue;
					}
	...
	public function getAdjacentByIdno(string $mode): ?array
	{
		...
		if (!$idno_fld || !$this->hasField("{$idno_fld}_sort_num")) {
			return null;
		}
		$pk = $this->primaryKey();
		$table = $this->tableName();
	...
	public static function find($pa_values, $pa_options = null)
	{
		...
		foreach ($pa_values as $vs_field => $va_field_values) {
			foreach ($va_field_values as $vn_i => $va_field_value) {
				...
				if (($vs_op == '=') && ($vm_value == '*') && (sizeof($pa_values) === 1)) {
					$vb_find_all = true;
					break (2);
				}
	...
	public function getRelatedItems($pm_rel_table_name_or_num, $pa_options = null)
	{
		...
		if (!empty($va_wheres)) {
			...
			error_log('va_wheres is still empty after condition');
		}
